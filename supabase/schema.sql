...
-- Anyone can insert profiles (via trigger)
...
-- Super Admins can manage learning paths
CREATE POLICY "Super Admins can manage learning paths"
...
-- Super Admins can manage courses
...
-- Enrolled users and admins can view lessons
...
-- Super Admins can manage lessons
...
-- Same as lessons
CREATE POLICY "Users can view enrolled videos"
  ...
  USING (
    ...
    EXISTS (
      ...
      WHERE l.id = videos.lesson_id AND e.user_id = auth.uid()
    ) OR
    EXISTS (
      SELECT 1 FROM profiles
      WHERE id = auth.uid() AND role = 'SUPER_ADMIN'
...
CREATE POLICY "Users can view enrolled materials"
  ...
  USING (
    ...
    EXISTS (
      SELECT 1 FROM enrollments e
      JOIN lessons l ON l.course_id = e.course_id
      WHERE l.id = materials.lesson_id AND e.user_id = auth.uid()
    ) OR
    EXISTS (
      SELECT 1 FROM profiles
      WHERE id = auth.uid() AND role = 'SUPER_ADMIN'
...
CREATE POLICY "Super Admins can manage materials"
...
-- Super Admins can manage certificates
...
-- Super Admins can manage newsletters
...
CREATE POLICY "Super Admins can view newsletter logs"
...
CREATE POLICY "Users can view public storage files"
...
CREATE POLICY "Super Admins can manage learning path courses"
...
-- Note: The super admin user must be created in Supabase Auth first
-- This script updates their profile to SUPER_ADMIN role

-- Update existing profile to Super Admin (run after creating auth user)
UPDATE profiles
...
VALUES (
  'Default Certificate',
  'Standard InnovaSci Open Academy certificate template',
  '/templates/certificate-default.png',
  TRUE
...
VALUES (
  ...
  'data-science-fundamentals',
  'Master the foundations of data science and machine learning',
  'This comprehensive learning path covers Python programming, statistics, machine learning, and data visualization. Perfect for beginners looking to start a career in data science.',
  'PUBLISHED'
...
VALUES (
  ...
  'This course is designed for beginners who want to learn Python programming. You will learn basic syntax, data types, control structures, functions, and object-oriented programming.',
  'BEGINNER',
  20,
  0,
  TRUE,
...
WHERE lp.slug = 'data-science-fundamentals' AND c.slug = 'introduction-to-python'
ON CONFLICT DO NOTHING;

-- Sample Lesson
INSERT INTO lessons (course_id, title, description, order_index, is_preview)
SELECT id, 'Getting Started with Python', 'Introduction to Python and setting up your development environment', 1, TRUE
FROM courses WHERE slug = 'introduction-to-python'
ON CONFLICT DO NOTHING;
